...
The goal of this thesis is to estimate volatility in financial markets. Let $y_{1:t} = \{y_1, y_2, \ldots, y_t\}$ denote the sequence of observed returns up to time $t \leq T$. Similarly, let $x_{0:t} = \{x_0, x_1, \ldots, x_t\}$ denote the sequence of latent states up to time $t$, where $x_0$ is the initial prior state \footnote{We indexed the latent states from $0$ to $T$, whereas the observed returns are indexed from $1$ to $T$. This way we want to emphasize that the initial state $x_0$ is the prior and we don't associate it with any observed return. One could define the indexing differently (e.g. observations also starting with $t=0$), but we decided to use this convention}.
...
Another objective is to estimate the model parameters $\theta$ from the observed data. This can be achieved by computing the marginal posterior distribution of the parameters:
\begin{equation}
...
\end{align}
where we used the Markov property of the latent state process. The joint posterior distribution can then be expressed as
\begin{equation}
