...
It is a state space model in which the hidden state is the tuple $X_t = (H_t, S_t)$. The log volatility $H_t$ is continuous, while the regime indicator $S_t$ is discrete and takes values in $\{1, 2, \ldots, K\}$, where $K$ is the number of regimes \cite{so1998stochastic}. The latent state space is therefore
\begin{equation}
...
\end{equation}
where $\mathcal{N}(\mu, \sigma^2)$ denotes the normal distribution with mean $\mu$ and variance $\sigma^2$. Such distribution admits a density function which we denote by $q(y_t | x_t, \theta) = q(y_t | h_t)$, as the observation density does not depend on the regime $s_t$ or any other model parameters $\theta$. 
...
Again such transition distribution admits a density function which we can write as
\begin{align}
...
In the case of unknown parameters, we first need to specify prior distributions for the model parameters $\theta$, which requires additional considerations. 

We assume that the prior distributions of the parameters are independent, so that the joint prior distribution can be factorized as
\begin{equation}
...
\begin{align}
    ...
    \mu_i &\sim \mathcal{TN}_{(0, \infty)}(0, 5^2), \quad i = 2, \ldots, K, \\
    2\phi - 1 &\sim \text{Beta}(6, 2), \\
    \eta &\sim \text{Gamma}(2, 5), \\
    P_{i,:} &\sim \text{Dirichlet}(\alpha_1, \ldots, \alpha_K), \quad i = 1, \ldots, K.
\end{align}
