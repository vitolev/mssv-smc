...
def test_auxiliary_particle_filter_lgm():
...
def test_auxiliary_particle_filter_mssv():
    rng = np.random.default_rng(42)
    ...
    # Generate synthetic observations
    T = 10
    state_true = model.sample_initial_state(theta)
    y = []
    for t in range(T):
    ...
    smoothing_traj, _ = apf.smoothing_trajectories(history, n_traj=5)
...
def test_conditional_auxiliary_particle_filter_lgm():
    rng = np.random.default_rng(42)
    ...
    history = apf.run_conditional(y, theta, true_path)
...
def test_conditional_auxiliary_particle_filter_mssv():
    rng = np.random.default_rng(42)
